...
if __name__ == '__main__':
    ...
    num_testcase = 1000000

    #MOT_total = np.zeros([num_testcase*len(basedir_total),20])

    names = ['Set Number', 'Max age', 'Min hits', 'Hung thres',  'rlA', 'rlB', 'rlC',
             'rlD', 'rlE', 'rlF', 'rlG', 'rlH', 'rlJ', 'rlK', 'rlL', 'rlM',
             'rlN', 'rlP', 'MOTA','MOTP']
    # MOT_total[0][1] = 'Test Case'
    # MOT_total[0][2] = 'MOTA'
    # MOT_total[0][3] = 'MOTP'
    MOT_total = names
    MOT_curr = np.zeros([1, 20])
    hung_thresh_total = np.array([0.01, 0.03, 0.05, 0.07])
    rng_thres = np.array([1, 10, 100])

     #     #FIXME : Do a coarse grid search
    #for num in range(1, num_testcase):


    for i in range(2):
        basedir = basedir_total[i]
        print(basedir)
        # basedir = '/home/wen/raw_data/JI_ST-cloudy-day_2019-08-27-21-55-47/10_jan/log_high/set_3/'
                                                    ...
                                                    for rlK in range(len(rng_thres)):
                                                        for rlL in range(len(rng_thres)):
                                                            for rlM in range(len(rng_thres)):
                                                                for rlN in range(len(rng_thres)):
                                                                    for rlP in range(len(rng_thres)):
                                                                        AMOTA = 0
                                                                        AMOTP = 0
                                                                        ...
                                                                        for i in range(2):
                                                                            ...
                                                                            if isReady == 1:
                                                                                ...
                                                                                # tuning
                                                                                P_0cr[0][0] *= rng_thres[rlH]
                                                                                P_0cr[1][1] = P_0cr[0][0]
                                                                                P_0cr[3][3] *= rng_thres[rlJ]
                                                                                P_0cr[4][4] *= rng_thres[rlK]
                                                                                P_0cr[5][5] = P_0cr[4][4]
                                                                                P_0cr[7][7] *= rng_thres[rlL]
                                                                                P_0cr[8][8] = P_0cr[7][7]
                                                                                ...
                                                                                # tuning
                                                                                P_0ibeo[0][0] *= rng_thres[rlM]
                                                                                P_0ibeo[1][1] = P_0ibeo[0][0]
                                                                                P_0ibeo[3][3] *= rng_thres[rlN]
                                                                                P_0ibeo[7][7] *= rng_thres[rlP]
                                                                                ...
                                                                                #        Rlidar, Qlidar, P_0lidar , Rcr, Qcr, P_0cr, Ribeo, Qibeo, P_0ibeo, radarCam_threshold, radar_offset, testPIXOR, testIBEO, testCamDar)
                                                                        ...
                                                                        MOT_curr[0][16] = rlN
                                                                        MOT_curr[0][17] = rlP
                                                                        MOT_curr[0][18] = AMOTA/len(basedir_total)
                                                                        MOT_curr[0][19] = AMOTP/len(basedir_total)
