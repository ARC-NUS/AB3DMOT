...
def classChecker(x):
    ...
    if x == "ca" or x == "Ca":  # car (van is also car)/Car
        ...
        classCheck = 4
    if x == "tr" or x == "Tr":  # truck (lorry is also truck)/Truck
        # color = tuple(color_range[5])
        classCheck = 5
    if x == "bu" or x == "Bus":  # bus/Bus
...
def check_iou_json_class(labels_json_path, tracker_json_path, thres_d=100., distance_metric="IOU", is_write=True,
    ...
    for index, labels in enumerate(labels_data):  # for each pcd/timestep labelled
        ...
        for row, column in indexes:
            ...
            if value < thres_d:  # FIXME signage different for IOU and eucledian?  if the IOU is < 100 then YAY .
                ...
                #print opts, tpts
                #total_dist += value
                total_dist_class[classCheck] += value
    ...
    for i in range(len(total_gt_class)):
    ...
    for i in range(len(total_ct_class)):
...
if __name__ == '__main__':
    ...
    MOTA, MOTP, MOTA_class, MOTP_class,  total_missed_class , total_gt_class, total_dist_class, MOTA_status, MOTP_status = check_iou_json_class(labels_json_path,
                                                                                                       tracker_json_path,
                                                                                                       thres_d,
                                                                                                       distance_metric)
    print MOTA, MOTP
    print total_missed_class , total_gt_class, MOTA_class, total_dist_class, MOTP_class
