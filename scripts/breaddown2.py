...
def get_tracker_json(pixor_json_name, pixor_stats_json, tracker_json_outfile, fused_pose_json, max_age=3, min_hits=2,
                     hung_thresh=0.05, Q=np.identity(10), is_write=True):
            ...
            for count, pcd in enumerate(data):
                ...
                for d in trackers:
                    if is_utm:
                        ...
                        obj_dict = {"width": d[1], "height": d[0], "length": d[2], "x": bl_x, "y": bl_y, "z": d[5],
                                    "yaw": d[6], "id": d[7]}
                    else:
                        obj_dict = {"width": d[1], "height": d[0], "length": d[2], "x": d[3], "y": d[4], "z": d[5],
                                    "yaw": d[6], "id": d[7]}
                        ...
                        for pmax_age in prange(4):
                            for pmin_hits in prange(4):
                                for ha_thresh in np.arange(0.1, 1.0, 0.4):
                                    ...
                                    tracker_params = "age" + str(max_age) + "_hits" + str(min_hits) + "_thresh" + str(
                                        ha_thresh)
                                    ...
                                    q_params = "_xy" + str(q_xy) + "_ori" + str(q_heading) + "_wx" + str(
                                        q_wx) + "_ly" + str(q_ly) + "_v" + str(q_v)
                                    ...
                                    get_tracker_json(pixor_json_name=pixor_json_name,
                                                     tracker_json_outfile=tracker_json_outfile,
                                                     fused_pose_json=fused_pose_json, max_age=max_age,
                                                     min_hits=min_hits, hung_thresh=ha_thresh, Q=Q)
    return True
...
if __name__ == '__main__':
    ...
    fused_pose_json = "/home/wen/raw_data/JI_ST-cloudy-day_2019-08-27-21-55-47/10_jan/log_high/set_1/fused_pose/fused_pose.json"
    ...
    if is_tuning:
        ...
        tracker_json_outfile = "./results/yltracker_set_1.json"
